...
/** Metadata for whichever route renders this view. */
export function earlyRegistrationMetadata(locale: string, path = ""): Metadata {
  const c = getContent(locale);
  const suffix = path ? `/${path}` : "";
  const url = `${SITE}/${locale}${suffix}`;
  return {
    title: c.meta.title,
...
export async function EarlyRegistrationView({ locale }: { locale: string }) {
  const c = getContent(locale);
      ...
      <footer className={s.footer}>
        <div className={s.container}>
          <div className={s.footerGrid}>
            ...
            <div>
              <h3>{c.footer.academyTitle}</h3>
              {c.footer.academyLinks.map((l) => (
                <a href={`/${locale}${l.href}`} key={l.href}>
                  {l.label}
                </a>
              ))}
            </div>
            <div>
              <h3>{c.footer.legalTitle}</h3>
              {c.footer.legalLinks.map((l) => (
                <a href={`/${locale}${l.href}`} key={l.href}>
                  {l.label}
                </a>
              ))}
              <h3 style={{ marginTop: 20 }}>{c.footer.contactTitle}</h3>
          ...
          {/* Statutory identity. Numbers print only when they are real — the
          ...
          <div className={s.copyright}>
            ...
            {(legal.crNumber || legal.vatNumber) && (
              <div className="num" dir="ltr" style={{ marginTop: 4 }}>
                ...
                {legal.crNumber && legal.vatNumber && "  ·  "}
                {legal.vatNumber && `${c.footer.vatLabel}: ${legal.vatNumber}`}
              </div>
            )}
            <div style={{ marginTop: 4 }}>
              {legal.isVatRegistered ? c.footer.vatIncluded : c.footer.priceFinal}
            </div>
