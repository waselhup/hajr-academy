/**
 * Applies the Checkout grade migration
 * (prisma/migrations-checkout-grade.sql) to the Supabase database.
 *
 * Mirrors apply-communication.ts: retries transient pooler drops with
 * a fresh PrismaClient per attempt. The SQL is fully idempotent, so
 * re-running after an outage is always safe.
 *
 * Run: npx tsx prisma/apply-checkout-grade.ts
 */
...
async function main() {
  const sql = readFileSync(join(__dirname, "migrations-checkout-grade.sql"), "utf8");
  ...
  console.log("✅ Checkout grade migration applied.");
}
