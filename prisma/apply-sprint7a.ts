/**
 * Applies the Sprint 7A migration (prisma/migrations-sprint7a.sql).
 * Idempotent. Run: npx tsx prisma/apply-sprint7a.ts
 *
...
async function main() {
  const sql = readFileSync(join(__dirname, "migrations-sprint7a.sql"), "utf8");
  const statements = splitStatements(sql);
  console.log(`Applying ${statements.length} Sprint 7A statements...`);
  for (let i = 0; i < statements.length; i++) {
    await applyStatement(statements[i], `stmt#${i + 1}`);
    console.log(`  ✓ stmt#${i + 1}`);
  }
  console.log("✅ Sprint 7A migration applied.");
}
