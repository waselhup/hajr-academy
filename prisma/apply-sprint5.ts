/**
 * Applies the Sprint 5 migration (prisma/migrations-sprint5.sql).
 * Idempotent. Run: npx tsx prisma/apply-sprint5.ts
 */
...
async function main() {
  const sql = readFileSync(join(__dirname, "migrations-sprint5.sql"), "utf8");
  const statements = splitStatements(sql);
  console.log(`Applying ${statements.length} Sprint 5 statements...`);
