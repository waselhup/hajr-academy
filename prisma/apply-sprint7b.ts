/**
 * Applies the Sprint 7B migration (prisma/migrations-sprint7b.sql).
 * Idempotent. Run: npx tsx prisma/apply-sprint7b.ts
 *
 * Sprint 7B = TeacherRating extensions + Gamification (StudentGamification,
...
async function main() {
  const sql = readFileSync(join(__dirname, "migrations-sprint7b.sql"), "utf8");
  const statements = splitStatements(sql);
  console.log(`Applying ${statements.length} Sprint 7B statements...`);
  for (let i = 0; i < statements.length; i++) {
    await applyStatement(statements[i], `stmt#${i + 1}`);
    console.log(`  ✓ stmt#${i + 1}`);
  }
  console.log("✅ Sprint 7B migration applied.");
}
