/**
 * Applies the Sprint 4 migration (prisma/migrations-sprint4.sql).
 * Idempotent. Run: npx tsx prisma/apply-sprint4.ts
 */
...
async function applyStatement(stmt: string, label: string): Promise<void> {
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    ...
    try {
      ...
      // Storage bucket inserts may fail if storage schema not accessible
      if (/schema "storage" does not exist|storage.buckets/i.test(msg) &&
          /INSERT INTO storage.buckets/i.test(stmt)) {
        console.warn(`  ⚠ ${label}: storage schema not accessible — skipping bucket insert.`);
        return;
      }
...
async function main() {
  const sql = readFileSync(join(__dirname, "migrations-sprint4.sql"), "utf8");
  const statements = splitStatements(sql);
  console.log(`Applying ${statements.length} Sprint 4 statements...`);
